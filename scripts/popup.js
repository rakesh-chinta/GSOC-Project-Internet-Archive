...
{  
  ...
  myWindow.document.write("<p><b>Description about the Extension</b><BR> This chrome extension allows the user to search for the archive history of any URL from within the search box, it also provides the top 5 recommendations. Lets the user save current pages in the archive of the wayback machine, lets the user view the recent and first version of the current URL, lets the user share the current url to 4 social networking websites namely: facebook, twitter, google plus and linkedin. Lets the user view the recent tweets about the current URL. The alexa and whois statistics of the current URL can also be found out. <BR><b> 1.Tweets feature:</b> <BR> The tweets feature was built by parsing the active URL from the omnisearchbox and searching the result on the twitter search platform on the basis of advanced search, there was an optional setting where we can view and set the boundaries of the dates from which the tweets can be viewed by. Now at the current release the tweets button displays the current top 10 tweets made on the current URL.<BR><b>2.Social sharing buttons:</b> <BR>The social sharing buttons are used by the user to share the current active URL to one's social media accounts.They currently support 4 social media platforms, namely: Facebook, Google plus, LinkedIn, and Twitter. <BR><b> 3.Integration with Wayback Machine's Site search:</b> <BR>The wayback machine's site search has been integrated within the extension, it works as follows: The user receives recommendation based on their input within the searchbox after typing in 3 characters, they are shown a limit of the top 5 search recommendations.The user has the option to click on one of the search recommendations and is redirected to Internet archives history where the URL is archived, or can continue typing it the URL and search manually based on his criteria for Save page now, recent version, first version or can use the search button to click and go to the Internet archives's archive history of the URL. <BR> <b>4.Alexa Statistics and Whois Statistics: </b><BR>Alexa and whois Statistics buttons parse the URL for the main part/root part and display the alexa and whois statistics given by alexa.com and whois search API respectively to the user. <BR><b>5.Automatic Archiving of URL's:</b> <BR>The display notifications of the auto-save features are explained as in 3 cases: displays to the user in 4 different modes based on the progress or current status or the URL. <BR> Modes: <BR>Mode1: Displays CHECKING, which indicates that the current URL is being checked whether if it was already archived in the wayback-machine, it then proceeds to the next steps   <BR>Mode2: shows a green signal stating YES if the webpage is already archived in the wayback machine. <BR>Mode3: shows NO, meaning the current webpage is not archived in the wayback machine. <BR>Mode4: If NO, then SAVING is shown indicating that the webpage is currently being archived and we get a final message of YES <BR>The final chrome extension built in GSOC was the combination of the advanced features built by @abhidas and I, on the base of the Internet Archive's chrome extension version 1.3.2, which was built by several contributors, given below. <BR><b>License</b> <BR>Copyright Internet Archive, 2017 AGPL-3 <BR><b>Credits</b> <BR>Richard Caceres, @rchrd2 <BR>Mark Graham, @markjohngraham <BR>Benjamin Mandel <BR>Kumar Yoges <BR>Anton, @ <BR>Abhidhas, @abhidas17695 <BR>Rakesh N Chinta, @rakesh-chinta </p>");
  myWindow.focus();        
}
/**
 * @description: about & support  
